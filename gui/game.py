...
class Score:
    ...
    def update(self, val=None):
        if val is not None and val != self.val :
            self.val_msg.change_message(str(val))
...
class TurretBar:
    ...
    def __init__(self):
        ...
        self.get_turret_buttons()

    def get_turret_buttons(self):
...
class GUI:
    ...
    def update(self, mouse_event, click_event):
        ...
        placed_turret = None
        if self.selected_turret is not None and click_event is not None:
                placed_turret, self.selected_turret = self.selected_turret, None
        # assign new selected after update of turret_bar
        selected_turret = self.turret_bar.update(mouse_event, click_event)
        if selected_turret is not None and click_event is not None:
            self.selected_turret = selected_turret
            self.selected_turret.iso_pos = click_event.pos
        ...
        self.next_wave.update(mouse_event, click_event)
    ...
    def display(self, screen):
        ...
        if self.selected_turret is not None:
            if self.undertile is not None:
                screen.blit(Tile.bright_patch, self.undertile.iso_rect)
